...
client.on('message', message => {
        if (message.content === "Qinv") {
            if(!message.channel.guild) return;
        let embed = new Discord.RichEmbed()
        .setAuthor(` ${message.author.username} `, message.author.avatarURL)      
        .setTitle(`اضغط هنا `)
        .setURL(`https://discordapp.com/api/oauth2/authorize?client_id=559118584228610048&permissions=8&scope=bot`)
...
client.on('message', message => {
     if (message.content === "Qsupport") {
     let embed = new Discord.RichEmbed()
  .setAuthor(message.author.username)
  .setColor("#9B59B6")
  .addField(" ** :gear: Server Support :gear: **" , "  **https://discord.gg/sSN7daB**")
...
client.on('ready', () => {
var x = client.channels.get("559142106933755936");
if (x) x.join();
});
...
Qplay <name> | لتشغيل القران

Qstop | لتوقيف القران وخروج البوت من الرووم 

Qskip | لتخطي القران
 
Qvol number | لتغيير الصوت 

Qpause | ايقاف بشكل موقت

Qresume | تكميل القران 

Qnp | لمعرف اي سورة مشتغلة

Qqueue | لمعرفة السور الاخرى التي طلبتها 
